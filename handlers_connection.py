"""Connection lifecycle for Apptio Cloudability Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from cloudability_client import ApptioCloudabilityClient
...
_SECRET = "cloudability_connections"
...
@chat.function(
    "connect_cloudability",
    "Connect Apptio Cloudability account via credentials.",
    action_type="write",
    chain_callable=True,
    event="cloudability-connector.connect_cloudability",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_cloudability(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Apptio Cloudability Connector."""
    client = ApptioCloudabilityClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Apptio Cloudability Account",
...
@chat.function(
    "list_connections",
    "List connected Apptio Cloudability accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_cloudability",
    "Disconnect Apptio Cloudability account.",
    action_type="write",
    chain_callable=True,
    event="cloudability-connector.disconnect_cloudability",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_cloudability(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
