"""Resource handlers for Apptio Cloudability Connector."""
...
async def _get_client(ctx, cid: str = ""):
    ...
    if not conn:
        return None, ActionResult.error("No active Apptio Cloudability connection", code="UNAUTHORIZED")
    return ApptioCloudabilityClient(api_key=conn["api_key"], base_url=conn.get("base_url", "")), None
...
async def audit_audience_health(params: ConnectionIdParams, ctx) -> ActionResult[AuditAudienceHealthResult]:
    ...
    return ActionResult.ok(AuditAudienceHealthResult(
        summary="Apptio Cloudability Audit bounce rates, spam complaints and list engagement metrics",
...
async def get_campaign_analytics(params: ConnectionIdParams, ctx) -> ActionResult[GetCampaignAnalyticsResult]:
    ...
    return ActionResult.ok(GetCampaignAnalyticsResult(
        summary="Apptio Cloudability Aggregated open rates, click rates and deliverability snapshot",
        metrics={"status": "healthy", "scanned_at": now_iso, "alerts": 0},
        timestamp=now_iso
    ))
