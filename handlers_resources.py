"""Resource handlers for Cloudability Connector."""
...
@chat.function("list_costs", "List costs in Cloudability.", action_type="read", chain_callable=True, event="cloudability-connector.list_costs", effects=["read:costs"], data_model=CostRecordList)
...
@chat.function("get_costrecord", "Get details of one CostRecord in Cloudability.", action_type="read", chain_callable=True, event="cloudability-connector.get_costrecord", effects=["read:costrecord"], data_model=CostRecordRecord)
...
@chat.function("audit_costrecord_health", "Audit health of Cloudability costs and connectivity.", action_type="read", chain_callable=True, event="cloudability-connector.audit_costrecord_health", effects=["read:audit"], data_model=AuditHealthReport)
async def audit_costrecord_health(ctx, params: ConnectionIdParams) -> ActionResult:
    ...
    try:
        ...
        return ActionResult.success({
            ...
            "summary": f"Cloudability healthy. Sampled {len(items)} costs."
        }, summary=f"Cloudability health check passed with {len(items)} costs.")
    except Exception as e:
        return ActionResult.error(f"Error auditing Cloudability health: {e}")
